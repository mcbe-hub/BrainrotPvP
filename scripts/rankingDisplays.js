...
function handleRanking(ranking, players) {
    const leaderboard = JSON.parse((world.getDynamicProperty(`ranking:${ranking.id}`) ?? "[]"));
    for (const player of players) {
        const score = player.getDynamicProperty(ranking.property);
    ...
    for (let i = 0; i < Math.min(10, leaderboard.length); i++) {
        ...
        if (player.value > 50000)
            stringDisplay = formatMoney(player.value);

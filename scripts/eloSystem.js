export function postKillElo(winner, loser) {
    const winnerElo = winner.getDynamicProperty("elo") ?? 1000;
    const loserElo = loser.getDynamicProperty("elo") ?? 1000;
    const K = 32; // K-factor
    // Expected scores
...
export function getEloString(elo) {
    if (elo >= 2500) {
        return `§g[§p${elo}§g]§r`;
    }
    if (elo >= 2000) {

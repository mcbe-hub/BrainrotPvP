...
export function playerExchangeUi(player, page = 0, items = fetchItems(), playerItems = false) {
    if (page == 0)
        payout(player);
    ...
    forceShow(ui, player).then((data) => {
        ...
        if (selection < 44) {
            ...
            switch (selection) {
                case 45:
                    if (page > 0)
                        playerExchangeUi(player, page - 1, items);
                    break;
                case 53:
                    if (items.length > 44)
                        playerExchangeUi(player, page + 1, items);
...
function listItem(player) {
    ...
    if (item) {
        ...
        ui.show(player).then(data => {
            if (data.canceled)
                return;
            const price = parseInt(data.formValues[0]);
            const properties = [];
            ...
            if (item.nameTag)
                exchangeItem.itemName = item.nameTag;
...
function buyItem(player, item) {
    ...
    ui.show(player).then(data => {
        ...
        if (data.selection == 11) {
            ...
            const seller = server.world.getEntity(item.sellerId);
            if (seller) {
...
function getPayouts() {
    const property = world.getDynamicProperty("payouts");
    let payouts = [];
    if (property)
        payouts = JSON.parse(property);
    return payouts;
}
...
function manageListing(player, item) {
...
function turnIntoItemStack(exchangeItem) {
    ...
    if (exchangeItem.itemName)
        item.nameTag = exchangeItem.itemName;
    return item;
}
...
function getPlayerItems(id, items = fetchItems()) {
...
function fetchItems() {
    ...
    for (const id of server.world.getDynamicPropertyIds()) {
        ...
        if (split[0] == "exchange") {
            itemArray.push(...JSON.parse(world.getDynamicProperty(id)));
        }
    }

...
export function startGame(player: server.Player) {
    const ui = new ModalFormData()
        ...
        .show(player).then((data) => {
            if (!data.canceled) {
                const bet = Math.floor(data.formValues[0] as number)
...
function blackjack(player: server.Player, deck: string[], bet: number, dealerHand: string[], playerHand: string[]) {
...
function finishGame(player: server.Player, deck: string[], bet: number, playerHand: string[], dealerHand: string[]) {
...
function willDealerHit(dealerHand: string[], playerHand: string[]): boolean {
    const playerValue = calculateHandValue(playerHand)
    const dealerValue = calculateHandValue(dealerHand)
    if (dealerValue < playerValue) return true
    return false
}
...
const randomizationArray: number[] = []
for(let i = 0; i<75; i++){
...
interface Lootbox {
    name: string
    items: {weight: number, item: server.ItemStack}[]
}

export function openLootBox(player: server.Player){
    new ActionFormData()
    .title('Lootbox')
    .body('Lootboxy')
    .button('Otwórz')
...
function getColorSquare(x: number){
    switch(x){
        case 1: return '§a■'
        case 2: return '§c■'
        case 3: return '§9■'
...
function getRandomItem(lootbox: Lootbox){
...
const legendaryLootbox: Lootbox = {
    name: "Legendary",
    items: [

...
export function startDiceGame(player: server.Player) {
    const ui = new ModalFormData()
        ...
        .show(player).then((data) => {
            if (!data.canceled) {
                const bet = data.formValues[0] as number;
                const isHardcore = data.formValues[1] as boolean;
